    ...
    public function processWaste(array $data, array $user)
    {
        ...
        try {
            ...
            foreach ($stocks as $index => $stock) {
                ...
                // MySQL evaluates assignments left to right: determine states before decreasing the quantity.
                $this->query(
                    "UPDATE waste_stocks
                     SET status = CASE WHEN quantity_kg - :quantity_kg_status <= 0 THEN 'inactive' ELSE status END,
                         storage_status = CASE WHEN quantity_kg - :quantity_kg_storage <= 0 THEN 'consumed' ELSE storage_status END,
                         quantity_kg = quantity_kg - :quantity_kg_update
                     WHERE id = :id",
                    [
