...
class WeighbridgeTransportController extends Controller
{
    ...
    public function store()
    {
        ...
        $errors=[];
        if(!in_array($data['origin_type'],['external_supplier','internal_farm'],true)){$errors['origin_type']='Origine invalide.';}
        if($data['origin_type']==='external_supplier'&&!ctype_digit($data['supplier_id'])){$errors['supplier_id']='Fournisseur obligatoire.';}
        if($data['origin_type']==='internal_farm'&&!ctype_digit($data['origin_site_id'])){$errors['origin_site_id']='Ferme d’origine obligatoire.';}
        ...
        try{$this->model('WeighbridgeTransport')->createTransport($data,Auth::user());flash('success','BT créé. Chargement enregistré et transport mis en transit.');}catch(Exception $e){flash('error',$e->getMessage());}
